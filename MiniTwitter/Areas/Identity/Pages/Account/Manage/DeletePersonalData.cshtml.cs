...
namespace MiniTwitter.Areas.Identity.Pages.Account.Manage
{
    public class DeletePersonalDataModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (!result.Succeeded)
            {
                throw new InvalidOperationException($"Unexpected error occurred deleting twitterUser with ID '{userId}'.");
            }

            await _signInManager.SignOutAsync();

            _logger.LogInformation("TwitterUser with ID '{UserId}' deleted themselves.", userId);
